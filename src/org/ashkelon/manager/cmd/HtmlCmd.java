...
public class HtmlCmd extends ProcessAPICmd
{
   ...
   {
      return "This html command is a new feature in ashkelon.  It processes " +
            "ashkelon's JSPs directly to produce static html pages.  The look " +
            "of the pages can be easily customized by modifying the JSP templates";
   }
   ...
   public void registerParameters() throws JSAPException
   {
      ...
      FlaggedOption outputdirOption = new FlaggedOption("outputdir")
                                          .setRequired(false)
                                          .setShortFlag('d')
                                          .setLongFlag("outputdir");

      outputdirOption.setHelp("Output directory:  base directory where produced " +
            " static html pages will be written to.");
      registerParameter(outputdirOption);


      Option apiOption = new UnflaggedOption("api").setRequired(true);

      apiOption.setHelp("API to populate, specified as an api.xml file or " +
            "as a maven project.xml file (does not work with all maven projects)");
   ...
   public void invoke(JSAPResult arguments)
   {
      ...
      try
      {
         ...
         String[] javadocargs = constructJavadocArgslist(arguments, 
                                                         apifilename,
                                                         api);

         // invoke javadoc (parms: programName, docletClassName, javadocargs)
   ...
   private String[] constructJavadocArgslist(JSAPResult arguments, 
                                             String apifilename, 
                                             API api)
   {
